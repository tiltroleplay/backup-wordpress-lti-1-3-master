...
/**
 * LTI logging helper function
 */
if (!function_exists('IMSGlobal\LTI\lti_log')) {
    function lti_log($message, $data = null) {
        $log = '[LTI ' . date('Y-m-d H:i:s') . '] ' . $message;
        if ($data !== null) {
            $log .= ' | Data: ' . (is_string($data) ? $data : print_r($data, true));
        }
        error_log($log);
    }
}
...
class LTI_Message_Launch {
    ...
    public function validate(array $request = null) {
        ...
        lti_log("=== LTI LAUNCH VALIDATION STARTED ===");
        ...
        lti_log("Request received", [
            'has_id_token' => isset($request['id_token']),
        ...
        try {
            ...
            lti_log("=== LTI LAUNCH VALIDATION FAILED ===", $e->getMessage());
            throw $e;
        }
    }
    ...
    private function get_public_key() {
        ...
        lti_log("Fetching public key from: " . $key_set_url);
        ...
        lti_log("Public key set fetched", ['keys_count' => isset($public_key_set['keys']) ? count($public_key_set['keys']) : 0]);

        if (empty($public_key_set)) {
            lti_log("ERROR: Failed to fetch public key set from " . $key_set_url);
            throw new LTI_Exception("Failed to fetch public key", 1);
        }

        // Find key used to sign the JWT (matches the KID in the header)
        $target_kid = $this->jwt['header']['kid'] ?? null;
        lti_log("Looking for key with KID: " . $target_kid);
        ...
        foreach ($public_key_set['keys'] as $key) {
            if ($key['kid'] == $target_kid) {
                try {
                    ...
                    // Get the actual key material from the Key object
                    $keyMaterial = $parsedKey->getKeyMaterial();
                    $keyDetails = openssl_pkey_get_details($keyMaterial);

                    lti_log("Public key parsed successfully for KID: " . $target_kid);
                    return $keyDetails;

                } catch(\Exception $e) {
                    lti_log("ERROR: Failed to parse key for KID " . $target_kid . ": " . $e->getMessage());
                    return false;
                }
            }
        }
        ...
        lti_log("ERROR: Unable to find public key with KID: " . $target_kid);
    ...
    private function validate_state() {
        ...
        $cookie_name = 'lti1p3_' . $state;
        $cookie_value = $this->cookie->get_cookie($cookie_name);

        lti_log("Validating state", [
            'state' => $state,
            'cookie_name' => $cookie_name,
            'cookie_exists' => ($cookie_value !== null)
        ]);

        if ($cookie_value !== $state) {
            lti_log("ERROR: State validation failed - cookie mismatch", [
                'expected' => $state,
                'got' => $cookie_value
            ]);
        ...
        lti_log("State validated successfully");
        return $this;
    }
    ...
    private function validate_jwt_signature() {
        ...
        lti_log("Validating JWT signature");

        // Fetch public key.
        $public_key = $this->get_public_key();

        if (!$public_key || !isset($public_key['key'])) {
            lti_log("ERROR: Public key not available for JWT validation");
        ...
        try {
            ...
            lti_log("JWT signature validated successfully");

        } catch(\Exception $e) {
            lti_log("ERROR: JWT signature validation failed: " . $e->getMessage());
            throw new LTI_Exception("Invalid signature on id_token: " . $e->getMessage(), 1);
        }
