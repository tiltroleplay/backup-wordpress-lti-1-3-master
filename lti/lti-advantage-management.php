...
class LTIAdvantageManagement
{
    ...
    function admin_add_css_js()
    {
        ...
        wp_register_script(
            'grades-management-js-admin',
            plugins_url('js/grades-management.js', __FILE__),
    ...
    public function add_admin_css($hook)
    {
        ...
        if (!empty($_GET['type']) && $_GET['type'] == 'comments') {
            ...
            wp_register_style(
                'grades-management-css-comments',
                plugins_url('lti/css/backend-comments.css', __FILE__),
    ...
    public function add_lti_grades_management_menu()
    {
        ...
        if (get_lti_show_gradebook_to_teachers() || get_lti_show_sync_members_to_teachers()) {
            ...
            add_menu_page(
                __('LTI Advantage', self::$DOMAIN),

                __('LTI Advantage', self::$DOMAIN),
                self::$CAPABILITY_EDITOR_ROLE,

                'lti_advantage',
                array($this, 'lti_advantage')
            );
    ...
    public function lti_grades_management_syncmembers()
    {
        ...
        <div id="grades-management-membership">
            ...
            <img class="waiting" src="<?php echo esc_url(admin_url('images/wpspin_light-2x.gif')) ?>" /><?php _e(
                   'Loading Memberships',
    ...
    //         if ($launch) {
    //             // Add this debug log right after retrieving the launch
    //             error_log("Launch data: " . print_r($launch->get_launch_data(), true));
    //         }

    //         if ($lti_user_id && $launch && $launch->has_ags()) {

    //             // Log endpoint claim (includes scopes)
    //             // $endpoint_claim = $launch->get_launch_data()['https://purl.imsglobal.org/spec/lti/claim/endpoint'];
    //             $endpoint_claim = $launch->get_launch_data()['https://purl.imsglobal.org/spec/lti-ags/claim/endpoint'] ?? [];
    //             $raw_scopes = $endpoint_claim['scope'] ?? null;

    //             // Defensive: if null, use default or log error
    //             if (!is_array($raw_scopes)) {
    //                 $raw_scopes = preg_split('/\s+/', trim($raw_scopes));
    //                 error_log("ERROR: AGS scopes missing from endpoint claim.");
    //                 // fallback: at minimum, include 'score' to attempt token
    //                 $scopes = ['https://purl.imsglobal.org/spec/lti-ags/scope/score'];
    //             }

    //             // Make a fresh array with all string values
    //             $scopes = array_values(array_map('strval', $raw_scopes));

    //             // Now pass a copy to avoid internal casting issues
    //             $scopes_copy = $scopes;

    //             error_log('Normalized scopes: ' . print_r($scopes, true));

    //             $grades = $launch->get_ags();
    //             // Use Reflection to access the service connector inside $grades
    //             $ref = new ReflectionClass($grades);
    //             $prop = $ref->getProperty('service_connector');
    //             $prop->setAccessible(true);
    //             $service_connector = $prop->getValue($grades);

    //             $launch_data = $launch->get_launch_data();
    //             $platform_id = $launch_data['iss'];
    //             $deployment_id = $launch_data['https://purl.imsglobal.org/spec/lti/claim/deployment_id'];

    //             // error_log('what is scopes now: ' . var_dump($scopes));
    //             // Now try fetching a token manually
    //             $token_response = $service_connector->get_access_token(
    //                 $platform_id,
    //                 $deployment_id,
    //                 $scopes_copy
    //             );

    //             error_log("AGS token response: " . print_r($token_response, true));

    //             // $token = $grades->get_service_access_token();
    //             // error_log("AGS token: " . print_r($token, true));
    ...
    //                 error_log("Moodle put_grade failed");
    //             } else {
    //                 error_log("Moodle put_grade succeeded");
    //             }
    ...
    //             error_log("AGS not available for user {$userid}");
    //         }
    ...
    //         error_log("Missing user id or grade in save_internal_grade");
    ...
    //     if (!$lti_user_id || !$launch || !$launch->has_ags()) {
    //         $ret['error'] = __('Don\'t allow to send back grades. Review LMS configuration', self::$DOMAIN);
    //         return $ret;
    //     }

    //     $grades = $launch->get_ags();

    //     // Reflection to access service connector
    //     $ref = new ReflectionClass($grades);
    //     $prop = $ref->getProperty('service_connector');
    //     $prop->setAccessible(true);
    //     $service_connector = $prop->getValue($grades);

    //     // Grab AGS endpoint claim
    //     $launch_data = $launch->get_launch_data();
    //     $endpoint_claim = $launch_data['https://purl.imsglobal.org/spec/lti-ags/claim/endpoint'] ?? [];

    //     $raw_scopes = $endpoint_claim['scope'] ?? null;

    //     // Defensive normalization
    //     if (empty($raw_scopes)) {
    //         error_log("ERROR: AGS scopes missing from launch. Using fallback 'score'.");
    //         $raw_scopes = ['https://purl.imsglobal.org/spec/lti-ags/scope/score'];
    //     }

    //     // Make sure it is a flat array of strings
    //     $scopes = array_values(array_map('strval', (array) $raw_scopes));
    //     error_log("Normalized scopes: " . print_r($scopes, true));
    ...
    //     // Fetch access token
    //     try {
    //         $token_response = $service_connector->get_access_token($scopes);
    //         error_log("AGS token response: " . print_r($token_response, true));
    //     } catch (\Throwable $e) {
    //         $ret['error'] = 'Failed to fetch LTI AGS token: ' . $e->getMessage();
    //         return $ret;
    //     }
    ...
    //     // Use reflection to get service connector
    //     $ref = new ReflectionClass($grades);
    //     $prop = $ref->getProperty('service_connector');
    //     $prop->setAccessible(true);
    //     $service_connector = $prop->getValue($grades);
    ...
    //     $endpoint_claim = $launch_data['https://purl.imsglobal.org/spec/lti-ags/claim/endpoint'] ?? [];
    //     $deployment_id = $launch_data['https://purl.imsglobal.org/spec/lti/claim/deployment_id'] ?? '';
    //     $scopes = $endpoint_claim['scope'] ?? [];

    //     if (!is_array($scopes)) {
    //         $scopes = is_string($scopes) ? preg_split('/\s+/', trim($scopes)) : (array) $scopes;
    //     }
    //     $scopes = array_values(array_map('strval', $scopes));

    //     error_log("=== LTI AGS DEBUG START ===");
    //     error_log("Deployment ID: " . $deployment_id);
    //     error_log("Normalized scopes: " . print_r($scopes, true));
    //     error_log("Endpoint claim full: " . print_r($endpoint_claim, true));
    //     error_log("=== LTI AGS DEBUG END ===");
    //     // --- DEBUG LOGGING END ---

    //     $scopes = ['https://purl.imsglobal.org/spec/lti-ags/scope/lineitem', 'https://purl.imsglobal.org/spec/lti-ags/scope/score'];
    //     $token_response = $service_connector->get_access_token($scopes);
    //     error_log("Token response for minimal scopes: " . print_r($token_response, true));
    ...
    //     // Debugging: log the grade object
    //     error_log("=== LTI AGS GRADE DEBUG START ===");
    //     error_log("Grade object: " . print_r($score, true));

    //     // Try to inspect what endpoint will be used
    //     $lineitem_url = null;
    //     if (method_exists($grades, 'get_lineitem')) {
    //         $lineitem_url = $grades->get_lineitem();
    //     } elseif (property_exists($grades, 'lineitem')) {
    //         $lineitem_url = $grades->lineitem;
    //     }

    //     error_log("Lineitem URL being used: " . $lineitem_url);

    //     // Send grade
    //     $success = $grades->put_grade($score, null);
    //     error_log("put_grade() returned: " . var_export($success, true));
    //     error_log("=== LTI AGS GRADE DEBUG END ===");
    ...
    function save_grade_lti()
    {
        ...
        if (!current_user_can(self::$CAPABILITY_EDITOR_ROLE)) {
            ...
            // $saveGrade = $this->save_internal_grade($userid, $grade, $comment);
            // if( $saveGrade ){
            //     $ret['result'] = true; 
            //     $ret['error'] = false;
            // } 
            $saveGrade = do_action('lti_send_grade', $userid, $grade, $comment);
            if ($saveGrade) {
                $ret['result'] = true;
                $ret['error'] = false;
            }
